...
public class UserReject extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                int i = sto.executeUpdate("update users set statuss='Rejected' where id='" + id + "'");
                System.out.println("test print==" + id);
                if (i != 0) {
                    
                        response.sendRedirect("UserDetails.jsp?Rejected");
                    } else {

                        System.out.println("failed");
                        response.sendRedirect("UserDetails.jsp?Failed");
                    }
                }
