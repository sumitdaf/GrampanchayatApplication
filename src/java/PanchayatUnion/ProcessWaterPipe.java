 ...
 * @author Beast
 */
public class ProcessWaterPipe extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String watuniqueid = "WATER" + num;

            Connection conn = SQLconnection.getconnection();
            Statement sto = conn.createStatement();

            try {
                int i = sto.executeUpdate("update water_con set statuss='Active',watuniqueid='" + watuniqueid + "',price='" + scost + "' where id='" + id + "'");
                System.out.println("test print==" + id);
                if (i != 0) {

                    response.sendRedirect("SetWaterPipe.jsp?Granted");
                } else {
